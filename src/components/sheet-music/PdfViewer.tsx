...
interface PdfViewerProps {
  url: string;
  /** 홈 등 좁은 카드용: iframe 높이 축소, 하단 안내 숨김 */
  compact?: boolean;
}
...
export function PdfViewer({ url, compact = false }: PdfViewerProps) {
  ...
  return (
    <div
      className={`w-full rounded-lg border border-stone-200 overflow-hidden bg-white ${compact ? "flex flex-col min-h-0" : ""}`}
    >
      <iframe
        src={fullUrl}
        title="PDF 뷰어"
        ...
        style={
          compact
            ? { height: 240, minHeight: 200, maxHeight: 260 }
