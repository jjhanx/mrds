...
### 추가
- **도구**: `osmd-universal-viewer/` — 별도 Vite 앱. MusicXML/MXL·NWC(→MusicXML, `pako` inflate). OSMD 가로 한 줄 스태프, Staff 다중 선택 재생, 파트별 GM 선택, 재생 중 음표 빨간색 강조(머리만·rAF 합침), 정지 시 스크롤로 재생 시작 스텝 이동, GM `stop` 중복 제거·StepQueue 빈틱 보완, soundfont-player + osmd-audio-player 벤더
