...
- 파트 선택: 악보 왼쪽 파트명 strip 클릭 또는 드롭다운 체크박스로 1개 이상 파트 선택, 선택된 파트만 재생, 파란색 하이라이트 (파트 1개 시 strip/버튼 숨김)
- 재생·악보 동기화: Tempo base 변환, 자동 스크롤; 빨간선 viewport 60px 고정, 끝에 닿으면 악보 따라 이동
