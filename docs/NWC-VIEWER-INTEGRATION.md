...
**mrds 맞춤 수정 사항:**
- **NWC 2.75 가사**: NWCTXT `|Lyrics|` `|Lyric1|` 파싱 + UTF-8 디코딩 (한글 가사 정상 표시)
- `?file=` 파라미터로 URL 로드
- `?file=` 시 임베드 모드: Open nwc, 샘플 선택, NWC to MusicXML, Parser, Layout, Size, Footer 숨김
- 한글 가사: lib/nwc2xml reader에서 고바이트 시 EUC-KR 우선 디코딩; Malgun Gothic·Noto Sans KR 폰트
- 악보 레이아웃: pageWidth 최소 1000px로 마디당 한 줄 완화
- 임베드 시 기본 줌 25%: 한 줄에 4마디 이상 보이도록 스케일 축소
- 줌 범위: 25%~100% (임베드 권장 구간)
- 파트 선택: [전체] [파트1] [파트2] ... 버튼으로 1개 이상 파트 선택 (클릭 토글), 선택된 파트만 재생, 파란색 하이라이트 (파트 1개 시 버튼 숨김)
- 재생·악보 동기화: Tempo base 변환, rAF 보간 스크롤, sync offset(-20ms); 빨간선 viewport 60px 고정, 끝에 닿으면 악보 따라 이동
- 스크롤 기반 시크: 악보를 스크롤바·드래그로 옮기면 빨간 선 위치가 갱신되고, Play 시 해당 위치에서 재생 시작
- 임베드 시 scroll 레이아웃: 각 Staff 한 줄에 길게, 오른쪽으로 스크롤
- 마디 길이: 가장 긴 Staff 기준으로 모든 Staff 맞춤 (빨간선 따라가기 용이)
