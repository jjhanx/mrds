...
- 한글 가사: EUC-KR/CP949 인코딩 지원, lyric 폰트에 Malgun Gothic·Noto Sans KR 등 한글 폰트 스택 적용
- 악보 레이아웃: pageWidth 최소 1000px로 마디당 한 줄 완화
- 임베드 시 기본 줌 25%: 한 줄에 4마디 이상 보이도록 스케일 축소
- 파트 선택: 여러 Voice 시 파트별 재생 선택 드롭다운
- 재생·악보 동기화: Tempo base(whole/half/quarter/eighth) 변환, 재생 시 악보 자동 스크롤
